from aws_cdk import core, aws_s3, aws_s3_deployment, aws_cloudfront, aws_route53, aws_route53_targets, aws_lambda, \
    aws_apigateway, aws_certificatemanager
...
class JukebikeStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, id: str, **kwargs) -> None:
        ...
        hosted_zone = aws_route53.HostedZone.from_hosted_zone_attributes(self, 'hostedZone', hosted_zone_id=hosted_zone_id, zone_name=hosted_zone_name)

        # set variables for backend
        search_spotify_code_location = "jukebike/backend/"
        ...
        #Certificate
        cert = aws_certificatemanager.DnsValidatedCertificate(self, "certificate_website", domain_name=site_domain, hosted_zone=hosted_zone, region="us-east-1")
        ...
        search_spotify_lambda = aws_lambda.Function(self, "SearchSpotifyLambda",
                                                    code=aws_lambda.Code.from_asset(search_spotify_code_location),
                                                    handler="searchspotify.handler",
                                                    runtime=aws_lambda.Runtime.PYTHON_3_7
                                                    )

        CfnOutput(self, "SearchSpotifyLambda_", value=search_spotify_lambda.function_arn)

        search_spotify_api = aws_apigateway.LambdaRestApi(
            self, 'SearchSpotifyEndpoint',
            handler=search_spotify_lambda,
        )

        # Publish API with custom domain name
        # Pre-Requirements:
        ...
        certificate = aws_certificatemanager.DnsValidatedCertificate(self, "domaincertificate", hosted_zone=hosted_zone, region='us-east-1', domain_name=certificate_domain, validation_method=aws_certificatemanager.ValidationMethod.DNS)
        ...
        domain.add_base_path_mapping(target_api=search_spotify_api, base_path="search")

        target = aws_route53_targets.ApiGatewayDomain(domain)
        record_target = aws_route53.RecordTarget.from_alias(target)
        alias_record = aws_route53.ARecord(self, 'aliasRecord', target=record_target, record_name=api_domain, zone=hosted_zone)
